...
from yolov3_tf2.models_00003 import (
...
def main(_argv):
    ...
    if FLAGS.multi_gpu:
        ...
        for physical_device in physical_devices:
            tf.config.experimental.set_memory_growth(physical_device, True)
            #tf.config.set_logical_device_configuration(physical_device, [tf.config.LogicalDeviceConfiguration(memory_limit=20000)])
        model, optimizer, loss, anchors, anchor_masks = setup_model()
    ...
    if FLAGS.mode == 'eager_tf':
        ...
        for epoch in range(1, FLAGS.epochs + 1):
            ...
            model.save_weights(
                'checkpoints/00003/yolov3_train_{}.tf'.format(epoch))
    else:

        callbacks = [
            ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=1, verbose=1),
            EarlyStopping(patience=3, verbose=1),
            ModelCheckpoint('checkpoints/00003/yolov3_train_{epoch}.tf',
        ...
        print('Total Training Time: %f' %end_time)
